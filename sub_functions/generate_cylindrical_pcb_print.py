...
def generate_cylindrical_pcb_print(coil_parts: List[CoilPart], input_args):
    ...
    if input_args.surface_is_cylinder_flag and input_args.make_cylndrical_pcb:
        if input_args.pcb_interconnection_method != 'spiral_in_out':
            for part_ind, coil_part in enumerate(coil_parts):
                ...
                for wire_part_ind in range(len(segment_starts)):
                    if wire_part_ind < len(segment_starts) - 1:
                    ...
                    pcb_parts.track_shape = pcb_track_shape

                coil_part.pcb_tracks.upper_layer.group_layouts.wire_parts = pcb_parts

        else:  # Generate the pcb form the spiral in/out tracks
            upper_layer = [None] * len(coil_parts)
            lower_layer = [None] * len(coil_parts)

            for part_ind, coil_part in enumerate(coil_parts):
                coil_part = coil_parts[part_ind]
                rot_cylinder_vertices = np.dot(rot_mat, coil_part.coil_mesh.vertices)
                ...
                upper_layer[part_ind] = [None] * len(coil_part.connected_group)
                lower_layer[part_ind] = [None] * len(coil_part.connected_group)
                ...
                for group_ind, connected_group in enumerate(coil_part.connected_group):
                    ...
                    for group_layer in ['upper', 'lower']:
                        ...
                        for wire_part_ind in range(len(pcb_parts)):
                            ...
                            pcb_parts[wire_part_ind]['polygon_track'] = Polygon(pcb_parts[wire_part_ind]['track_shape'].T)
                ...
                coil_part['pcb_tracks']['upper_layer'] = upper_layer
                coil_part['pcb_tracks']['lower_layer'] = lower_layer
