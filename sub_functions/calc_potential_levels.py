...
def calc_potential_levels(coil_parts: List[CoilPart], combined_mesh, input_args):
    ...
    for part_ind, coil_part in enumerate(coil_parts):
        if level_set_method == "primary":
            ...
            for part in coil_parts:
                part.contour_step = (np.max(part.stream_function) - np.min(part.stream_function)) / (num_levels - 1 + 2 * level_offset)
                part.potential_level_list = np.arange(num_levels) * part.contour_step + (np.min(part.stream_function) + level_offset * part.contour_step)
            primary_surface_ind = 0
