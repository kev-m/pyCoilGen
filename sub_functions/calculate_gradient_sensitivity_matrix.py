...
def calculate_gradient_sensitivity_matrix(coil_parts, target_field, input):
    ...
    for part_ind in range(len(coil_parts)):
        if not input.temp_evalution.use_preoptimization_temp:
            ...
            gradient_sensitivity_matrix = np.zeros((3, num_target_points, num_nodes))

            verts1 = coil_parts[part_ind].coil_mesh.vertices[:, coil_parts[part_ind].coil_mesh.faces[0, :]]
            verts2 = coil_parts[part_ind].coil_mesh.vertices[:, coil_parts[part_ind].coil_mesh.faces[1, :]]
            verts3 = coil_parts[part_ind].coil_mesh.vertices[:, coil_parts[part_ind].coil_mesh.faces[2, :]]
            ...
            for node_ind in range(num_nodes):
                ...
                for tri_ind in range(coil_parts[part_ind].basis_elements[node_ind].triangle_points_ABC.shape[0]):
                    ...
                    for gauss_ind in range(len(gauss_weight)):
                        l_x = x1 * u_coord[gauss_ind] + x2 * v_coord[gauss_ind] + x3 * (1 - u_coord[gauss_ind] - v_coord[gauss_ind])
                ...
                gradient_sensitivity_matrix[:, :, node_ind] = np.vstack((DBzdx, DBzdy, DBzdz)) * biot_savart_coeff * plate_thickness
