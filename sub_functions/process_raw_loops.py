...
def biot_savart_calc_b2(wire_elements: np.ndarray, target_f: TargetField):
    """
    Calculate the magnetic field using Biot-Savart law for wire elements given as a sequence of coordinate points.
    ...
    Returns:
        ndarray: The magnetic field calculated using Biot-Savart law with shape (3, num_tp).
    """
    # Calculate B field with Biot-Savart's law using wire elements as coordinate points
    num_tp = target_f.coords.shape[1]
    # To avoid memory problems, split the curve into several parts
    track_part_length = 1000
    ...
    if wire_elements.shape[1] > track_part_length:
        ...
        wire_parts = []
        for parts_ind in range(len(track_part_inds) - 1):
            coord = wire_elements[:, track_part_inds[parts_ind]:track_part_inds[parts_ind + 1]]
            seg_coords = (coord[:, :-1] + coord[:, 1:]) / 2
            currents = coord[:, 1:] - coord[:, :-1]
    ...
    # Calculate the magnetic field with Biot-Savart's law
    b_field = np.zeros((3, num_tp))

    for wire_part in wire_parts:
        target_p = target_f.coords[:, :, np.newaxis]
        cur_pos = wire_part.seg_coords[:, np.newaxis, :]
        cur_dir = wire_part.currents[:, np.newaxis, :]
        R = target_p - cur_pos

        len_R = (1 / np.linalg.norm(R, axis=0)**3)[np.newaxis, :, :]
        len_R = np.tile(len_R, (3, 1, 1))
        dB = 1e-7 * np.cross(cur_dir, R, axisa=0, axisb=0, axisc=0) * len_R
        b_field += np.sum(dB, axis=2)
