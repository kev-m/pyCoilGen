...
def find_group_cut_position(loop_group: TopoGroup, group_center: np.ndarray, mesh: Mesh, b_0_direction: np.ndarray, cut_plane_definition) -> List[CutPosition]:
    ...
    for loop_ind in range(len(loop_group.loops)):
        ...
        if len(cut_position.cut_point.v) == 0:
            ...
            cut_position.cut_point.v = np.hstack((loop_group.loops[loop_ind].v[:, min_ind], loop_group.loops[loop_ind].v[:, max_ind]))
            cut_position.cut_point.uv = np.hstack((loop_group.loops[loop_ind].uv[:, min_ind], loop_group.loops[loop_ind].uv[:, max_ind]))
